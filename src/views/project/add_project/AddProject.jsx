...
export const AddProject = () => {
    ...
    useEffect(() => {
        if (!esNuevo, id !== 'nuevo') {
          const getProduct = async () => {
            setLoading(false);
            const resp = await fetchConToken(`projects/${id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        const url = esNuevo ? "projects" : `projects/${id}`;
        ...
        if (resp.status === 201 || resp.status === 200) {
          Swal.fire({
            title: "Éxito",
            text: `Se ha ${esNuevo ? 'agregado' : 'editado'} el proyecto exitosamente`,
            icon: "success",
    ...
    return (
        <div>
            ...
            <Title
                title={esNuevo ? "Agregar proyecto" : "Editar proyecto"}
            />
