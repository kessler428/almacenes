...
export const AddCode = () => {
    ...
    useEffect(() => {
        if (!esNuevo, id !== 'nuevo') {
          const getProduct = async () => {
            setLoading(false);
            const resp = await fetchConToken(`codes/${id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        const url = esNuevo ? "codes" : `codes/${id}`;
        ...
        if (resp.status === 201 || resp.status === 200) {
          Swal.fire({
            title: "Éxito",
            text: `Se ha ${esNuevo ? 'agregado' : 'editado'} el código exitosamente`,
            icon: "success",
    ...
    return (
        <div>
            ...
            <Title
                title={esNuevo ? "Agregar Código" : "Editar Código"}
            />
